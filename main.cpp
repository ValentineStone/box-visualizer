...
struct WorkerThread {
    ...
    void setDataSource(std::string dataSource) {
        ...
        } else if (dataSource.ends_with(".bag")) {
            rs2::config rs_config;
            rs_config.enable_device_from_file(dataSource);
            rs_profile = rs_pipeline.start(rs_config);
            pipelineStarted = true;
        } else if (dataSource.ends_with("distances.bin")) {
                ...
                if (pipelineStarted) {
                    ...
                    for (const rs2::frame& frame: frameset) {
                        ...
                        if (stream_type == RS2_STREAM_DEPTH) {
                            std::cout << "rs2::depth_frame " << frame.get_frame_number() << "\r\n";
...
#pragma omp parallel for
                            ...
                            drawColor(color_copy, frame_width, frame_height, 90);
                        }
                    }
    ...
    void presentFromFiles(std::string distances_path) {
        std::filesystem::path intr_path = std::regex_replace(distances_path, std::regex("distances.bin"), "intrinsics.bin");
        if (loaded_distances)
            delete loaded_distances;
    ...
    void processDepth(uint16_t *depth_data, size_t frame_width, size_t frame_height, rs2_intrinsics &intr, float depth_units) {
        ...
        std::stringstream ss;
        ss << "Lookat (" << lookat_x << ',' << lookat_y << ") depth = " << lookat_depth;
    ...
    void drawColor(uint8_t* data, size_t width, size_t height, int rotate = 0) {
        cv::Mat img(cv::Size(width, height), CV_8UC3, (void*)data, cv::Mat::AUTO_STEP);
        cv::namedWindow("Color");
        cv::imshow("Color", img);
        delete[] data;
    }
    void drawRgb(const std::string& label, uint8_t* data, size_t width, size_t height, int rotate = 0) {
        cv::Mat img(cv::Size(width, height), CV_8UC3, (void*)data, cv::Mat::AUTO_STEP);
...
int main(int argc, char *argv[]) {
    ...
    WorkerThread wt;
    if (args.size() < 2) {
        std::cout << "Usage: " << args[0] << " <data_source>\r\n";
        return 1;
    }
    wt.setDataSource(args[1]);
    wt.run();
